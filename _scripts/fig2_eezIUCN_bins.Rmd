...
Request to output these results as a table with the percent of species in each species group in each redlist category and EEZ bin
...
rl.wide <- redlist_dat %>%
  pivot_wider(id_cols=c(tax_group_2, n_spp), 
              names_from=category,
              names_prefix="RedList_",
              values_from=perc_cat, # values are the percent of mapped species in each category 
              values_fill=0)

eez.wide <- eez_dat %>%
  pivot_wider(id_cols=c(tax_group_2, n_spp), 
              names_from=eez_bins_2,
              names_prefix="EEZ_", # these columns are the bins, so eez_10_25 == 10-25 jurisdictions
              values_from=perc_eez,
              values_fill=0)
head(eez.wide)
eez.wide <- eez.wide %>% janitor::clean_names()

comb.wide <- left_join(rl.wide, eez.wide, by = c("tax_group_2", "n_spp"))
comb.wide

comb.wide <- comb.wide %>%
  rename(count_species=n_spp, species_group=tax_group_2)

write_csv(comb.wide, "_results/fig2_resultstable.csv")
